...
#define IDX_SUB_CAM 1
    ...
    if (On)
    {
        ...
        if ((pinSetIdx == 1) && currSensorName && (0 == strcmp(SENSOR_DRVNAME_HI708_YUV,currSensorName))) 
        {
            PK_DBG("[CAMERA SENSOR] kdCISModulePowerOn get in---GC2145_YUV sensorIdx:%d; pinSetIdx=%d\n",SensorIdx, pinSetIdx);
            if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D2, VOL_1800/*VOL_2800*/,mode_name)) //IO
            {
            ...
            if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D, VOL_1800,mode_name)) //DVDD no use
            {
                PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
                //return -EIO;
                goto _kdCISModulePowerOn_exit_;
       ...
       /*[BUGFIX] ADD-END by sz.chengming.xiang for PR-1021927 2015/06/15*/
        ...
        else if ((pinSetIdx == 0) && currSensorName && (0 == strcmp(SENSOR_DRVNAME_S5K5E2YA_MIPI_RAW,currSensorName))) 
        {
            ...
            if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D, VOL_1200,mode_name)) //DVDD no use
            {
                PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
                //return -EIO;
                goto _kdCISModulePowerOn_exit_;
        ...
        else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_GC2355_RAW,currSensorName))) 
        {
            PK_DBG("[CAMERA SENSOR] kdCISModulePowerOn get in---GC2145_YUV sensorIdx:%d; pinSetIdx=%d\n",SensorIdx, pinSetIdx);
            if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D2, VOL_1800/*VOL_2800*/,mode_name)) //IO
            {
            ...
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdxTmp][IDX_PS_CMRST])
            {
                ...
                if(mt_set_gpio_out(pinSet[pinSetIdxTmp][IDX_PS_CMPDN],pinSet[pinSetIdxTmp][IDX_PS_CMPDN+IDX_PS_ON]))
                {
                    PK_DBG("[CAMERA LENS] set gpio failed!! \n");   //high == power down lens module
                }
                //RST pin

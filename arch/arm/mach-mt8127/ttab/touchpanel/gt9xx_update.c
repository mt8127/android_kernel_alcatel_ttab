...
static u8 gup_get_ic_fw_msg(struct i2c_client *client)
{
    ...
    for (retry = 0; retry < 2; retry++)
    {
        ...
        {
            GTP_ERROR("The check sum in ic is error.");
...
#define FW_LITTLE_SS51_LENGTH      (12 * 0x400)    // 12K
...
s32 gup_hold_ss51_dsp(struct i2c_client *client)
{
    ...
    while(retry++ < 200)
    {
        ...
        if (power_is_down)
        {
            GTP_DEBUG("System power is down, exit hold ss51 & dsp.");
            return FAIL;
        }
    #endif
        // step4:Hold ss51 & dsp
...
static u8 gup_check_update_file_fl(struct i2c_client *client, st_fw_head *fw_head, char *path)
{
    ...
    after_912_1020 = 0;
    if (!memcmp(fw_head->pid, "912", 3))
    {
        if (fw_head->vid > 0x1020)
        {
            after_912_1020 = 1;
            GTP_DEBUG("After 912_1020, config length set to 228");
        }
    }
...
#if GTP_FL_LITTLE_SYSTEM
// blocksize: 256Bytes
u8 gup_burn_ss51_block(struct i2c_client *client, s32 block_section, s32 fw_block, u16 *fw_chksum)
{
    s32 ret = 0;
    s32 index = 0;
    s32 i = 0;

    GTP_DEBUG("Burn big ss51 block%d section%d (size: 256Bytes)", fw_block, block_section);
    ...
    case 7:
    case 8:
        index = FW_HEAD_LENGTH + (8 + fw_block-1)* 1024 * 2 + (block_section-1) * 256;
        break;

    case 9:
    case 10:
    case 11:
        index = FW_HEAD_LENGTH + (16 - fw_block) * 1024 * 2 + (block_section-1) * 256;
        break;
    default:
        GTP_ERROR("Invalid firmware block: %d", fw_block);
        return FAIL;
    }

    ret = i2c_write_bytes(client, GTP_REG_BUFFER + (block_section - 1) * 256,
            &gtp_default_FW_fl[index], 256);

    if (ret < 0)
    {
        GTP_ERROR("Failed to burn ss51 block%d section%d (size: 256Bytes)!", fw_block, block_section);
        return FAIL;
    }
    ...
    if (block_section == 1)
    {
        ...
        for (i = 0; i < (2 * 1024); i += 2)
        {
            chksum += ((gtp_default_FW_fl[index + i]) << 8) + (gtp_default_FW_fl[index + i + 1]);
        }
        *fw_chksum = chksum;
    }
    return SUCCESS;
...
u8 gup_burn_ss51_seg_b(struct i2c_client *client, s32 size, u16 *fw_chksum)
{
    s32 ret = 0;
    s32 i = 0;
    u16 chksum = 0;

    GTP_DEBUG("Burn big ss51 Seg B(size: %dK)", size);

    ret = i2c_write_bytes(client, GTP_REG_SS51_SEG_B,
            &gtp_default_FW_fl[FW_HEAD_LENGTH], size * 1024);

    if (ret < 0)
    {
        GTP_ERROR("Failed to Burn ss51 Seg B(size: %dK)", size);
        return FAIL;
    }

    for (i = 0; i < (size * 1024); i += 2)
    {
        chksum += ((gtp_default_FW_fl[FW_HEAD_LENGTH + i]) << 8) + gtp_default_FW_fl[FW_HEAD_LENGTH + i + 1];
    }
    *fw_chksum = chksum;
    return SUCCESS;
}

static u8 gup_download_fw_little_ss51(struct i2c_client *client)
{
    s32 ret = 0;
    GTP_INFO("Download little ss51");

    GTP_DEBUG("Select Bank0.");

    ret = gup_set_ic_msg(i2c_client_point, _bRW_MISCTL__SRAM_BANK, 0x00);

    if (ret <= 0)
    {
        GTP_ERROR("Select bank0 fail.");
        return FAIL;
    }

    // Just write
    ret = i2c_write_bytes(client, 0xC000,
                &gtp_default_FW_fl[FW_HEAD_LENGTH + FW_SS51_LENGTH + FW_DSP_LENGTH], FW_LITTLE_SS51_LENGTH);


    if (ret < 0)
    {
        GTP_ERROR("Failed to write little ss51.");
        return FAIL;
    }
...
#if GTP_FL_LITTLE_SYSTEM
    if (GTP_FL_PWR_RESUME_BURN == dwn_mode)
    {
        return gup_download_fw_little_ss51(client);
    }
#endif

    if (GTP_FL_FW_BURN == dwn_mode)
    {
        GTP_DEBUG("[download_fw_ss51]Begin download ss51 firmware---->>");
    }
    else
    {
        GTP_DEBUG("[download_fw_ss51]Begin check ss51 firmware----->>");
    }
...
{
	return __gup_download_fw_dsp(client, dwn_mode, FW_DSP_LENGTH);
}
...
s32 gup_fw_download_proc(void *dir, u8 dwn_mode)
{
    ...
    if (GTP_FL_PWR_RESUME_BURN == dwn_mode)
    {
        GTP_INFO("[fw_download_proc]Begin poweron resume little system download.....");
    }
#endif
    if(GTP_FL_FW_BURN == dwn_mode)
    {
...
int gup_reload_fw_dsp(void *dir, u8 dwn_mode)
{
	...
	while (retry++ < 5) {
		ret = __gup_download_fw_dsp(i2c_client_point, dwn_mode, FW_DSP_LENGTH / 2); //reload 2k dsp code
		if (FAIL == ret){
	...
	ret = i2c_write_bytes(i2c_client_point, 0x801F, &value, 1); // clear gesture mode reg 
  if (ret == -1)
		return FAIL;
	else
		return SUCCESS;

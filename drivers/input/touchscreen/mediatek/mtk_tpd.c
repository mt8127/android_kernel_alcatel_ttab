...
int gesture_value;
char tpd_firmware_version_val[5];
...
>>>>>>> int_mtk8127-topwise-kd-dint_20150320
...
static ssize_t firmware_version_val_show(struct device* dev, struct device_attribute* attr, char* buf) {
	printk(KERN_ERR"caoyang test read!\n");
	return snprintf(buf, PAGE_SIZE, "%s\n", tpd_firmware_version_val);

}

static ssize_t config_version_val_show(struct device* dev, struct device_attribute* attr, char* buf) {
	printk(KERN_ERR"caoyang test read!\n");
	return snprintf(buf, PAGE_SIZE, "%x\n", tpd_config_version);

}

static ssize_t tpd_x_line_val_show(struct device* dev, struct device_attribute* attr, char* buf) {
	printk(KERN_ERR"caoyang test read!\n");
	return snprintf(buf, PAGE_SIZE, "%d\n", tpd_x_line);

}

static ssize_t tpd_y_line_val_show(struct device* dev, struct device_attribute* attr, char* buf) {
	printk(KERN_ERR"caoyang test read!\n");
	return snprintf(buf, PAGE_SIZE, "%d\n", tpd_y_line);
...
static DEVICE_ATTR(firmware_version, S_IRUGO | S_IWUSR, firmware_version_val_show, NULL);
static DEVICE_ATTR(cfg_version, S_IRUGO | S_IWUSR, config_version_val_show, NULL);
static DEVICE_ATTR(tpd_xline, S_IRUGO | S_IWUSR, tpd_x_line_val_show, NULL);
static DEVICE_ATTR(tpd_yline, S_IRUGO | S_IWUSR, tpd_y_line_val_show, NULL);
...
//add by caoyang 
    if(device_create_file(&pdev->dev, &dev_attr_gesture_val) < 0) {
	printk(KERN_ALERT"caoyang creat attribute val.");                
     }
     if(device_create_file(&pdev->dev, &dev_attr_firmware_version) < 0) {
	     printk(KERN_ALERT"caoyang creat attribute val.");                
     }
     if(device_create_file(&pdev->dev, &dev_attr_cfg_version) < 0) {
	     printk(KERN_ALERT"caoyang creat attribute val.");				 
     }
     if(device_create_file(&pdev->dev, &dev_attr_tpd_xline) < 0) {
	     printk(KERN_ALERT"caoyang creat attribute val.");				 
     }

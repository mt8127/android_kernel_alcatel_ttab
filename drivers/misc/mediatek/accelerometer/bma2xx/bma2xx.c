...
/**********add leon zhu******************/
/*----------------------------------------------------------------------------*/
static int BMA2xx_JudgeTestResult(struct i2c_client *client)
{
	struct bma_i2c_data *obj = obj_i2c_data;
	int res = 0;
	int index =0;
	s16  acc[BMA_AXES_NUM];
	int  self_result;


agen:
	memset(acc, 0, sizeof(int)*BMA_AXES_NUM);
	if(res = bma_read_raw_data(client, acc))
	{        
	...
	{			
		...
		acc[BMA_AXIS_X] = acc[BMA_AXIS_X] * 9807 / 64;
		acc[BMA_AXIS_Y] = acc[BMA_AXIS_Y] * 9807 / 64;
		acc[BMA_AXIS_Z] = acc[BMA_AXIS_Z] * 9807 / 64;
		
		printk("1 step: %d %d %d\n", acc[0],acc[1],acc[2]);
		
		self_result = acc[BMA_AXIS_X]*acc[BMA_AXIS_X] 
			+ acc[BMA_AXIS_Y]*acc[BMA_AXIS_Y] 
			+ acc[BMA_AXIS_Z]*acc[BMA_AXIS_Z];
		...
		//if ( (self_result>48112033) && (self_result<163580913) ) //between 0.55g and 1.7g 
		if ( (self_result>45000000) && (self_result<170000000) ) //between 0.55g and 1.7g 
		{												 
			GSE_ERR("BMA2XX_JudgeTestResult successful\n");
			return 0;
		}
		else
		{
		    	GSE_ERR("BMA2XX_JudgeTestResult failt\n");
			if (index++ < 5)
				goto agen;
		   	return -EINVAL;
		}
...
/*----------------------------------------------------------------------------*/
static int BMA2XX_InitSelfTest(struct i2c_client *client)
{
	int err = 0;
	err = bma_set_datarate(client,
		(enum BMA_DATARATE_ENUM)BMA_DATARATE_10HZ);
...
{   
	...
	GSE_LOG("NORMAL:\n");
	bma_set_powermode(obj->client, BMA_NORMAL_MODE);

	/*initial setting for self test*/
	BMA2XX_InitSelfTest(obj->client);
	GSE_LOG("SELFTEST:\n");    

	if(!BMA2xx_JudgeTestResult(obj->client))
	{
...
static DRIVER_ATTR(self, S_IWUSR | S_IRUGO,
		show_selftest_value, store_selftest_value);
/**************end*****************/
		...
		unsigned long arg)
{
	...
	printk("GSENSOR_IOCTL_SET_CALI sensor_data.x = %d y =%d z = %d obj->sensitivity = %d\n",sensor_data.x,sensor_data.y,sensor_data.z,obj->sensitivity);
		/* convert: m/s^2 -> LSB */
	/**********add by Zero**********/
	#if 0
	...
	if (sensor_data.x > 0)
		sensor_data.x = sensor_data.x + 153;
	else if(sensor_data.x < 0)
		sensor_data.x = sensor_data.x - 153;

	if (sensor_data.y > 0)
		sensor_data.y = sensor_data.y + 153;
	else if(sensor_data.y < 0)
		sensor_data.y = sensor_data.y - 153;

	if (sensor_data.z > 0)
		sensor_data.z = sensor_data.z + 153;
		...
		cali[BMA_AXIS_Z] = sensor_data.z *
			obj->sensitivity / GRAVITY_EARTH_1000;
	#endif
	printk("GSENSOR_IOCTL_SET_CALI cali[BMA_AXIS_X]= %d y =%d z = %d\n",cali[BMA_AXIS_X],cali[BMA_AXIS_Y],cali[BMA_AXIS_Z]);
	/************end*****************/

...
/* / for lower cpu loading to skip some frame in lcm video mode,
SKIP_FRAME(define in 2-->5) means wfd driver will skip a lcm frame per PERFORMANCE_TUNING_SKIP_FRAME. */
...
do { \
	if (wfd_log_on) { \
		pr_info("[wfd]%s,#%d ", __func__, __LINE__); \
		pr_info(fmt, ##arg); \
	} \
} while (0)
...
struct  {
	...
	/* whether memory is forced to be rotated in Camera, no need to set orientation again */
...
/* static struct hdmi_video_buffer_list
*hdmi_search_video_buffer(struct hdmi_video_buffer_info *buffer_info, struct file *file); */
...
void init_wfd_mmp_events(void)
{
	if (WFD_MMP_Events.WFD == 0) {
		...
		WFD_MMP_Events.OverlayDone =
		    MMProfileRegisterEvent(WFD_MMP_Events.WFD, "OverlayDone");
		WFD_MMP_Events.UpdateRequest =
		    MMProfileRegisterEvent(WFD_MMP_Events.WFD, "UpdateRequest");
		WFD_MMP_Events.DDPKBitblt =
		    MMProfileRegisterEvent(WFD_MMP_Events.WFD, "DDPKBitblt");
...
static int hdmi_overlay_config_kthread(void *data)
{
	...
	for (;;) {
		...
		wait_event_interruptible(hdmi_overlay_config_wq,
					 atomic_read(&hdmi_overlay_config_event));
		...
		if (p->lcm_is_video_mode) {
			...
			hdmi_buffer_lcdw_id_tmp =
			    (hdmi_buffer_lcdw_id + 1) % hdmi_temp_buffer_number;
			addr =
			    overlay_dst_mva +
			    hdmi_buffer_lcdw_id_tmp * p->lcm_width * p->lcm_height * 3;
			...
			addr =
			    overlay_dst_mva +
			    hdmi_buffer_lcdw_id * p->lcm_width * p->lcm_height * 3;
...
static void hdmi_update_impl(void)
{
	...
	if (p->is_reconfig_needed || p->is_wfd_suspend) {
		...
		for (i = 0; i < hdmi_temp_buffer_number; i++) {
			memset((void *)(ddp_dst_va + i * p->hdmi_width * p->hdmi_height * 3), 0x00,
			       p->hdmi_width * p->hdmi_height);
			memset((void *)(ddp_dst_va + i * p->hdmi_width * p->hdmi_height * 3 +
					p->hdmi_width * p->hdmi_height), 0x80,
			       p->hdmi_width * p->hdmi_height);
		}
		if (p->is_reconfig_needed) {
			/* is_reconfig_needed = false */
			p->is_reconfig_needed = false;
		}
	}

	if (p->is_wfd_suspend) {
		MMProfileLogEx(WFD_MMP_Events.DDPKBitblt, MMProfileFlagStart, hdmi_buffer_write_id,
			       0);
		hdmi_buffer_write_id = ((hdmi_buffer_write_id + 1) % hdmi_temp_buffer_number);

		WFD_PUT_NEW_BUFFER();
		wake_up_interruptible(&external_display_getbuffer_wq);
		MMProfileLogEx(WFD_MMP_Events.DDPKBitblt, MMProfileFlagEnd, hdmi_buffer_write_id,
			       0);
	...
	if ((p->lcm_is_video_mode) && (hdmi_buffer_lcdw_cnt % skipperframe == 0)) {
		WFD_LOG("skip frame %d, id--%d from %d\n", skipperframe, hdmi_buffer_lcdw_id_tmp,
			hdmi_buffer_lcdw_cnt);
		up(&hdmi_update_mutex);
		return;
	}
	...
	ovl_read_id =
	    ((hdmi_buffer_lcdw_id + (hdmi_temp_buffer_number - 1)) % hdmi_temp_buffer_number);
	...
	portraitOutput = ((pddp.orientation == 0 || pddp.orientation == 180)
			  && p->lcm_height > p->lcm_width) || ((pddp.orientation == 90
								|| pddp.orientation == 270)
							       && p->lcm_height < p->lcm_width);
	...
	/* mediatek, this is for IT only */
	{
		...
		Bitmap.pData =
		    (void *)overlay_dst_va +
		    p->lcm_width * p->lcm_height * 3 *
		    ((hdmi_buffer_lcdw_id +
		      (hdmi_temp_buffer_number - 1)) % hdmi_temp_buffer_number);
		Bitmap.bpp = 24;
	...
	if (portraitOutput) {
		...
		if (p->lcm_height > p->lcm_width)
			pddp.dstW =
			    ALIGN_TO(p->lcm_width * p->hdmi_height / p->lcm_height * scaling / 100,
				     4);
		else
			pddp.dstW =
			    ALIGN_TO(p->lcm_height * p->hdmi_height / p->lcm_width * scaling / 100,
				     4);
		...
		if (pddp.dstFormat == eRGB888_K) {
			pddp.dstAddr[0] =
			    ddp_dst_mva +
			    hdmi_buffer_write_id * p->hdmi_width * p->hdmi_height * 3 +
			    (p->hdmi_height - pddp.dstH) / 2 * p->hdmi_width * 3 + (p->hdmi_width -
										    pddp.dstW) / 2 *
			    3;
			pddp.dstAddr[1] = 0;
			pddp.dstAddr[2] = 0;
			...
			pddp.dstAddr[0] =
			    ddp_dst_mva +
			    hdmi_buffer_write_id * p->hdmi_width * p->hdmi_height * 3 +
			    (p->hdmi_height - pddp.dstH) / 2 * p->hdmi_width + (p->hdmi_width -
										pddp.dstW) / 2;
			pddp.dstAddr[1] =
			    ddp_dst_mva +
			    hdmi_buffer_write_id * p->hdmi_width * p->hdmi_height * 3 +
			    p->hdmi_width * p->hdmi_height + (p->hdmi_height -
							      pddp.dstH) / 2 * p->hdmi_width / 4 +
			    (p->hdmi_width - pddp.dstW) / 2 / 2;
			pddp.dstAddr[2] =
			    ddp_dst_mva +
			    hdmi_buffer_write_id * p->hdmi_width * p->hdmi_height * 3 +
			    p->hdmi_width * p->hdmi_height / 4 * 5 + (p->hdmi_height -
								      pddp.dstH) / 2 *
			    p->hdmi_width / 4 + (p->hdmi_width - pddp.dstW) / 2 / 2;
	...
	WFD_LOG("ts:sec=%d, nsec=%d\n", timestamp[hdmi_buffer_write_id].tv_sec,
		timestamp[hdmi_buffer_write_id].tv_usec);
...
static HDMI_STATUS hdmi_drv_init(void)
{
	...
	ret = m4u_alloc_mva(M4U_CLNTMOD_WDMA,
			    overlay_dst_va, ovl_dst_buffer_size, 0, 0, &overlay_dst_mva);
	if (ret != 0) {
	...
	m4u_dma_cache_maint(M4U_CLNTMOD_WDMA,
			    (void const *)overlay_dst_va, ovl_dst_buffer_size, DMA_BIDIRECTIONAL);

	ret = m4u_alloc_mva(M4U_CLNTMOD_WDMA, ddp_dst_va, ddp_dst_buffer_size, 0, 0, &ddp_dst_mva);
	if (ret != 0) {
		pr_info("m4u_alloc_mva() fail!\n");
		return HDMI_STATUS_OK;
	}
	...
	m4u_dma_cache_maint(M4U_CLNTMOD_WDMA,
			    (void const *)ddp_dst_va, ddp_dst_buffer_size, DMA_BIDIRECTIONAL);
...
static HDMI_STATUS hdmi_drv_deinit(void)
{
	...
	ret = m4u_dealloc_mva(M4U_CLNTMOD_WDMA,
			      overlay_dst_va, ovl_dst_buffer_size, overlay_dst_mva);
	WFD_LOG("overlay_dst_va mva dealloced\n");

	ret = m4u_dealloc_mva(M4U_CLNTMOD_WDMA, ddp_dst_va, ddp_dst_buffer_size, ddp_dst_mva);
...
/*static*/ void wfd_setorientation(int orientation)
{
	...
	if (down_interruptible(&hdmi_update_mutex)) {
		pr_info("[hdmi][HDMI] can't get semaphore in %s\n", __func__);
		return;
	}
...
static long hdmi_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case MTK_EXT_DISPLAY_SET_MEMORY_INFO:
		{
			...
			if (copy_from_user
			    (&info, (void __user *)argp, sizeof(struct ext_memory_info))) {
			...
			WFD_LOG
			    ("MTK_EXT_DISPLAY_SET_MEMORY_INFO, width=%d, height=%d, buffer number=%d, bpp=%d\n",
			...
			ovl_dst_buffer_size =
			    ...
			    hdmi_temp_buffer_number;
			ddp_dst_buffer_size =
			    p->hdmi_width * p->hdmi_height * 3 * hdmi_temp_buffer_number;
			...
			for (i = 0; i < hdmi_temp_buffer_number; i++) {
				memset((void *)(ddp_dst_va +
						i * p->hdmi_width * p->hdmi_height * 3), 0x00,
				       p->hdmi_width * p->hdmi_height);
				memset((void *)(ddp_dst_va +
						i * p->hdmi_width * p->hdmi_height * 3 +
						p->hdmi_width * p->hdmi_height), 0x80,
				       p->hdmi_width * p->hdmi_height);
			}
	...
	case MTK_EXT_DISPLAY_GET_BUFFER:
		{
			...
			MMProfileLogEx(WFD_MMP_Events.Getbuffer, MMProfileFlagStart,
				       wfd_pattern_output_index, 0);
			...
			if (wfd_pattern_output) {
				...
				{
					memset((void *)(ddp_dst_va +
							wfd_pattern_output_index * p->hdmi_width *
							p->hdmi_height * 3), 0x00,
					       p->hdmi_width * p->hdmi_height);
					memset((void *)(ddp_dst_va +
							wfd_pattern_output_index * p->hdmi_width *
							p->hdmi_height * 3 +
					...
					memset((void *)(ddp_dst_va +
							...
							p->hdmi_height * 3 +
							wfd_pattern_output_index * p->hdmi_width *
							p->hdmi_height / 8), 0xFF,
					       p->hdmi_width * p->hdmi_height / 8);
				}
				...
				wfd_pattern_output_index =
				    ((wfd_pattern_output_index + 1) % hdmi_temp_buffer_number);

				ret = copy_to_user(argp, &buf, sizeof(struct ext_buffer));

				MMProfileLogEx(WFD_MMP_Events.Getbuffer, MMProfileFlagEnd,
					       buf.ts_sec, buf.ts_nsec);
			...
			if (p->is_wfd_suspend) {
				memset((void *)(ddp_dst_va + p->hdmi_width * p->hdmi_height * 3),
				       0x00, p->hdmi_width * p->hdmi_height);
				memset((void *)(ddp_dst_va + p->hdmi_width * p->hdmi_height * 3 +
						p->hdmi_width * p->hdmi_height), 0x80,
				       p->hdmi_width * p->hdmi_height);
				get_buffer_id = 0;
			...
			WFD_LOG("get buffer, id=%d, ts: sec=%d, usec=%d\n", buf.id,
				timestamp[buf.id].tv_sec, timestamp[buf.id].tv_usec);

			ret = copy_to_user(argp, &buf, sizeof(struct ext_buffer));

			MMProfileLogEx(WFD_MMP_Events.Getbuffer, MMProfileFlagEnd, 2,
				       hdmi_buffer_write_id);
			...
			/* mediatek, this is for IT only */
			{
				...
				Bitmap.pData =
				    (void *)ddp_dst_va +
				    buf.id * p->hdmi_width * p->hdmi_height * 3;
				Bitmap.bpp = 24;
				MMProfileLogMetaBitmap(WFD_MMP_Events.Getbuffer, MMProfileFlagPulse,
						       &Bitmap);
			}
#endif
			return ret;
	...
	default:
		{
			pr_info("[hdmi][HDMI] arguments error\n");
			break;
		}
	}
...
static int hdmi_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	pr_info("[hdmi_mmap] vma->vm_pgoff=0x%08x\n", (unsigned int)(vma->vm_pgoff));
	pr_info("[hdmi_mmap] vma->vm_start=0x%08x\n", (unsigned int)(vma->vm_start));
	pr_info("[hdmi_mmap] vma->vm_end=0x%08x\n", (unsigned int)(vma->vm_end));
	pr_info("[hdmi_mmap] vma->vm_page_prot=0x%08x\n", (unsigned int)(vma->vm_page_prot));
	pr_info("[hdmi_mmap] vma->vm_flags=0x%08x\n", (unsigned int)(vma->vm_flags));
...
const struct file_operations wfd_fops = {
	.owner = THIS_MODULE,
...
static int hdmi_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		pr_info("[hdmi]alloc_chrdev_region fail\n");
		return -1;
	}
	...
	hdmi_overlay_config_task =
	    kthread_create(hdmi_overlay_config_kthread, NULL, "hdmi_overlay_config_kthread");
...
static int __init hdmi_init(void)
{
	...
	if (platform_device_register(&hdmi_device)) {
		/* add mark for coding style */
		pr_info("[hdmi]failed to register hdmi device");
	}
	if (platform_driver_register(&hdmi_driver)) {
		pr_info("[hdmi]failed to register hdmi driver\n");
		return -1;
	}

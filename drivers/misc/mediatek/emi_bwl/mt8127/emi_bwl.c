...
/*
 * ddr_type_show: sysfs ddr_size file show function.
 * @driver:
 * @buf: the string of ddr type
 * Return the number of read bytes.
 */
...
/*
 * ddr_size_store: sysfs ddr_size file store function.
 * @driver:
 * @buf:
 * @count:
 * Return the number of write bytes.
 */
static ssize_t ddr_size_store(struct device_driver *driver, const char *buf, size_t count)
{
  /*do nothing*/
  return count;
}

DRIVER_ATTR(ddr_size, 0644, ddr_size_show, ddr_size_store);
/*
...
static int __init emi_bwl_mod_init(void)
{
  ...
  if (ret) {
    xlog_printk(ANDROID_LOG_ERROR, "EMI/BWL", "fail to create DRAM_SIZE sysfs file\n");
  }
#if 0

...
UINT32 LCD_DisableAllLayer(UINT32 vram_start, UINT32 vram_end)
{
    ...
    return layer_enable;
#else
    Disp_Ovl_Engine_Dump_layer_info(mtkfb_instance, (OVL_CONFIG_STRUCT*)cached_layer, NULL, NULL);

    for (id = 0; id < DDP_OVL_LAYER_MUN; id++) {
        if (cached_layer[id].layer_en == 0)
            continue;

        if (cached_layer[id].addr >= vram_start &&
            cached_layer[id].addr < vram_end)
        {
            DISP_LOG_PRINT(ANDROID_LOG_INFO, "LCD", "  not disable(%d)\n", id);
            layer_enable |= (1 << id);
            continue;

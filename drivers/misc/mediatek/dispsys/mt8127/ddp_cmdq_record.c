...
void cmdq_dump_command_buffer(void* buffer, uint32_t size)
{
    ...
    #if 0
    printk("[CMDQ] --> cmdq_dump_command_buffer, buffer[0x%08x], size[%d]\n", buffer, size); 
    ...
    printk("[CMDQ] --> cmdq_dump_command_buffer, buffer[0x%08x], size[%d], intrCount[%d]\n", buffer, size, instrCount); 
    ...
    for(i = 0;  i < instrCount; i = i + 1)
    {
        ...
        uint32_t indexB = 2 * i;
        printk("[CMDQ] ...inst[%3d] = 0x%08x, inst[%3d] = 0x%08x\n", (indexA), pCMD[indexA], (indexB), pCMD[indexB]);
    }
    
    #endif
    
    printk("[CMDQ] <-- cmdq_dump_command_buffer\n"); 
    return; 
}

//--------------------------------------------------------------------------------------

static CMDQ_EVENT_ENUM cmdq_rec_mutex_from_scenario(CMDQ_SCENARIO_ENUM scn)
{
...
static uint32_t cmdq_rec_flag_from_scenario_32bit(CMDQ_SCENARIO_ENUM scn)
{
    ...
    printk("REC: %s, scenario:%d, flag:%d\n", __FUNCTION__, scn, flag);
...
{
    CMDQ_EVENT_ENUM waitToken = CMDQ_SYNC_TOKEN_INVALID;
    if (NULL == handle)
    {
        return -EFAULT;
    }
...
int32_t cmdq_rec_finalize_command(cmdqRecHandle handle, bool loop)
{
    ...
    if (!handle->finalized)
    {
        ...
        uint32_t secReg = DISP_REG_CMDQ_MM_SECURITY_CLR; 
        uint32_t flag = 0x0; 
        uint32_t i = 0;
        /*
        ...
        {
            flag  = (flag) | (0x1 << 0);
            printk("[CMDQ] REC: cmdq_rec_finalize_command, sec flag = 0x%08x, gBypassSecAccessControl:%d\n", flag, gBypassSecAccessControl); 
...
{
    uint32_t flag = 0x0; 
    uint32_t engineFlag = handle->engineFlag; 
                                ...
                                bool          isSecure)
{
    ...
    CMDQ_MSG("pHandle[0x%08x], set secure = %d\n", handle, isSecure);
    handle->secData.isSecure = isSecure; 

    // insert access control at the head of command
    //uint32_t secReg = DISP_REG_CMDQ_MM_SECURITY;
    uint32_t secReg = DISP_REG_CMDQ_MM_SECURITY_SET; 
    uint32_t flag = 0x0; 
    uint32_t i = 0; 
    /*
    ...
    //    flag  = (flag) | (0x1 << 16);
        flag  = (flag) | (0x1 << 0);
        printk("[CMDQ] sec flag: 0x%08x, gBypassSecAccessControl:%d \n", flag, gBypassSecAccessControl); 
    ///    cmdqRecWrite(handle, IO_VIRT_TO_PHYS(secReg) , flag, ~0); 
    }
    */
    return 0; 

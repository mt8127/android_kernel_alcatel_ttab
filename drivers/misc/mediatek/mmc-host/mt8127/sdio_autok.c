...
static U_AUTOK_INTERFACE_DATA g_autok_data[AUTOK_VCORE_SCAN_NUM][MAX_AUTOK_DAT_NUM];
...
ReTuneMatrix:
    ...
    /* In case total cycle in pad delay */
    ...
    P_AUTOK_CMD_TUNE_RES_T pCmdTuneRes)
{
    ...
    findLBound:
            ...
            if ((autok_rdata_scan[CKGENLeftBound[x]].BoundReg2_S > 0) &&
                (autok_rdata_scan[CKGENLeftBound[x]].BoundReg2_E == 0))
                IntBound = autok_rdata_scan[CKGENLeftBound[x]].BoundReg2_S; 
        }
        else {
        ...
        if (FBoundFound) {
            ...
            /* Get internal bound if LBound NOT found */
            ...
            AUTOK_PRINT("Cur. base full boundary is %d\r\n", FBoundCnt);
    ...
    ChngBaseCK:
        ...
        if (!autok_rdata_scan[baseCKGEN].BoundReg1_S && 
            ...
            if (RealCKGEN > CKGENMax) {
                AUTOK_PRINT("[WARN]Can NOT tune to the best point due to CKGEN limit!\r\n");
                ...
                if (autok_rdata_scan[RealCKGEN].Reg1Cnt && !autok_rdata_scan[RealCKGEN].BoundReg1_S) {
                     ...
                     * It is most probably caused by SSSS corner
                     */
                    if ((TinyMar > AUTOK_TINY_MAR_TH_BTM) && fSSSS) {
         ...
         * is compressed by too large output delay 
         */
        ...
        /* In case of Right boundary at CKGEN = 0 */
            ...
            if (RBoundPad >= (DataPassWin/2)) {
                ...
                if (IntBound) {
                    ...
                    /* To avoid the pad delay too big */
                    if (RealPadDelay >= 16) {
                        RealCKGEN++;
                        RealPadDelay -= DIV_CEIL_FUNC(MIN_CLK_GEN_DELAY_IN_PS, MIN_PAD_DELAY_IN_PS);
                    }
            ...
            /* In case of un-enough margin */
                ...
                if ((NewMargin/2 > (int)0) && (NewMargin/2 >= (int)(RBoundPad + 2))) {
                    ...
                    if (IntBound)
                        RealPadDelay = IntBound - NewMargin/2;
                    else
                        RealPadDelay = 31 - NewMargin/2;
                }
                else {
                    RealCKGEN = 0;
                    RealPadDelay = 0;
            ...
            if (RealCKGEN > CKGENMax) {
                ...
                if (IntBound) {
                    ...
                    if (IntBound > RealPadDelay) {
                        ...
                        if ((DataPassWin/2 - RealPadDelay - IntBound)/2 < IntBound)
                            RealPadDelay = IntBound - (DataPassWin/2 - RealPadDelay - IntBound)/2;
                        else
                            RealPadDelay = 0;
                        
                    }
                    
                    AUTOK_PRINT("Try to select the best margin!\r\n");
                }
            }
        }

        if ((RealPadDelay >= IntBound) && IntBound) {
            AUTOK_PRINT("[ERR] RD internal boundary is too large, should enlarge the CKGEN!!!\r\n");
            AUTOK_ERR();
        }
...
int 
msdc_autok_apply_param(struct msdc_host *host, void *pData, int len, unsigned int vcore_uv_off)
{
    U_AUTOK_INTERFACE_DATA *pAutok;
    unsigned int vcore_sel, vcore_uv, fIdent = 0;
    
    if(len != (sizeof(U_AUTOK_INTERFACE_DATA)*MAX_AUTOK_DAT_NUM*autok_vcore_scan_num))
        return -1;
    ...
    pAutok += (vcore_sel*MAX_AUTOK_DAT_NUM);
    ...
    AUTOK_PRINT("msdc_autok_apply_param...\r\n");
...
int msdc_autok_stg1_cal(struct msdc_host *host, unsigned int offset_restore)
{
    ...
    memset(g_autok_data, 0, 
        sizeof(U_AUTOK_INTERFACE_DATA)*MAX_AUTOK_DAT_NUM*autok_vcore_scan_num);
    ...
    for (idx = 0; idx < autok_vcore_scan_num; idx++) {
        ...
        if (autok_vcore_scan_num > 1) {
            autok_vcore_set(g_autok_vcore_sel[idx]);
            autok_get_current_vcore_offset();
        }
    
        autok_tuning_parameter_init(host, TUNING_STG1, g_autok_data[idx]);
        res = autok_tune_algorithm(host, TUNING_STG1, g_autok_data[idx]);

        if (!res)
            autok_show_parameters(host, g_autok_data[idx]);
        else {
            AUTOK_PRINT("[ERR]msdc_autok_stg1_cal returns %d\r\n", res);
            goto exit;
        }

        g_autok_data[idx][E_AUTOK_VERSION].version = AUTOK_VERSION_NO;
        g_autok_data[idx][E_AUTOK_FREQ].freq = host->mclk;
    }
...
*               -2 else error
*************************************************************************/
int msdc_autok_stg2_cal(struct msdc_host *host, void *pData, int len, unsigned int vcore_uv_off)
{
    unsigned int vcore_sel, vcore_uv, fIdent = 0;
    U_AUTOK_INTERFACE_DATA *pAutok;
    E_RESULT_TYPE res= E_RESULT_ERR;
    ...
    #ifdef MT6290
    if((pAutok + E_AUTOK_FREQ)->freq != host->sclk)
    {
        AUTOK_PRINT("Now operation freq(%d) not meet autok freq(%d)\r\n", host->sclk, (pAutok + E_AUTOK_FREQ)->freq);
        return -2;
    }
    #else
    ...
    if (!res) {
        ...
        /* Copy to global variable for on-line tuning */
        if (((unsigned int)g_autok_data != (unsigned int)pData)/* && fIdent*/)
            memcpy((void *)g_autok_data, pData, len);
    }
    else

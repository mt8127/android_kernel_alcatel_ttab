...
static void bq24297_shutdown(struct platform_device *dev)
{
    printk("[bq24297_shutdown] driver shutdown \n");
...
/**********************************************************
  ...
  *
  *********************************************************/
int bq24297_read_byte(kal_uint8 cmd, kal_uint8 *returnData)
{
    char     cmd_buf[1]={0x00};
    char     readData = 0;
    int      ret=0;

    mutex_lock(&bq24297_i2c_access);
    ...
    {    
        ...
        mutex_unlock(&bq24297_i2c_access);
        return 0;
    }
...
int bq24297_write_byte(kal_uint8 cmd, kal_uint8 writeData)
{
    char    write_data[2] = {0};
    int     ret=0;
...
/**********************************************************
  ...
  *
  *********************************************************/
kal_uint32 bq24297_read_interface (kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    printk("[bq24297_read_interface] Reg[%x]=0x%x\n", RegNum, bq24297_reg);
    
    bq24297_reg &= (MASK << SHIFT);
    *val = (bq24297_reg >> SHIFT);    
    printk("[bq24297_read_interface] Val=0x%x\n", *val);

    return ret;
}

kal_uint32 bq24297_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    kal_uint8 bq24297_reg = 0;
    int ret = 0;
    ...
    ret = bq24297_read_byte(RegNum, &bq24297_reg);
    //printk("[bq24297_config_interface] Reg[%x]=0x%x\n", RegNum, bq24297_reg);
    
    bq24297_reg &= ~(MASK << SHIFT);
    bq24297_reg |= (val << SHIFT);

    ret = bq24297_write_byte(RegNum, bq24297_reg);
    //printk("[bq24297_config_interface] Write Reg[%x]=0x%x\n", RegNum, bq24297_reg);

    // Check
    //bq24297_read_byte(RegNum, &bq24297_reg);
    //printk("[bq24297_config_interface] Check Reg[%x]=0x%x\n", RegNum, bq24297_reg);
...
void bq24297_dump_register(void)
{
    ...
    for (i=0;i<bq24297_REG_NUM;i++)
    {
        ...
        printk("[bq24297_dump_register] Reg[0x%X]=0x%X\n", i, bq24297_reg[i]);        
    }
}
...
{
    printk("[show_bq24297_access] 0x%x\n", g_reg_value_bq24297);
    return sprintf(buf, "%u\n", g_reg_value_bq24297);
}
static ssize_t store_bq24297_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    int ret=0;
    char *pvalue = NULL;
    ...
    printk("[store_bq24297_access] \n");
    
    if(buf != NULL && size != 0)
    {
        printk("[store_bq24297_access] buf is %s and size is %d \n",buf,size);
        reg_address = simple_strtoul(buf,&pvalue,16);
        
        if(size > 3)
        {        
            reg_value = simple_strtoul((pvalue+1),NULL,16);        
            printk("[store_bq24297_access] write bq24297 reg 0x%x with value 0x%x !\n",reg_address,reg_value);
        ...
        {    
            ...
            printk("[store_bq24297_access] read bq24297 reg 0x%x with value 0x%x !\n",reg_address,g_reg_value_bq24297);
            printk("[store_bq24297_access] Please use \"cat bq24297_access\" to get value\r\n");
        }        
    }    
    return size;
}
...
{    
    ...
    printk("******** bq24297_user_space_probe!! ********\n" );
...
static struct i2c_board_info __initdata i2c_bq24297 = { I2C_BOARD_INFO("bq24297", (0xd6>>1))};

static int __init bq24297_init(void)
{    
    int ret=0;
    
    printk("[bq24297_init] init start\n");
    
    i2c_register_board_info(BQ24297_BUSNUM, &i2c_bq24297, 1);

    if(i2c_add_driver(&bq24297_driver)!=0)
    {
        printk("[bq24297_init] failed to register bq24297 i2c driver.\n");
    }
    else
    {
        printk("[bq24297_init] Success to register bq24297 i2c driver.\n");
    }

    // bq24297 user space access interface
    ret = platform_device_register(&bq24297_user_space_device);
    if (ret) {
        printk("****[bq24297_init] Unable to device register(%d)\n", ret);
        return ret;
    }    
    ret = platform_driver_register(&bq24297_user_space_driver);
    if (ret) {
        printk("****[bq24297_init] Unable to register driver (%d)\n", ret);
        return ret;
    }

...
extern unsigned int *mt8127_power_gs_audio_playback;
extern unsigned int mt8127_power_gs_audio_playback_len;
...
void mt_power_gs_dump_audio_playback(void)
{
    mt_power_gs_compare("Audio Playback",                                                   \
                        mt8127_power_gs_audio_playback, mt8127_power_gs_audio_playback_len, \
...
#else
static int dump_audio_playback_read(struct file *flip, char __user *buf, size_t count, loff_t *offset)
{
    int len = 0;
    char *p = buf;
...
static const struct file_operations audio_playback_proc_fops = {
    .owner = THIS_MODULE,
    .read = dump_audio_playback_read,
};
#endif
...
static int __init mt_power_gs_audio_playback_init(void)
{
    ...
    #else
        mt_entry = proc_create("dump_audio_playback", S_IRUGO | S_IWUSR | S_IWGRP, mt_power_gs_dir, &audio_playback_proc_fops);
        if (!mt_entry) 
        {
            printk("[%s]: create proc file /proc/mt_power_gs/dump_audio_playback failed\n", __FUNCTION__);
        }
    #endif
    }
...
MODULE_DESCRIPTION("MT8127 Power Golden Setting - Audio Playback");

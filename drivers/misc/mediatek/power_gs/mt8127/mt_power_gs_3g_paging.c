...
extern unsigned int *mt8127_power_gs_3g_paging;
extern unsigned int mt8127_power_gs_3g_paging_len;
...
void mt_power_gs_dump_3g_paging(void)
{
    mt_power_gs_compare("3G Paging",                                              \
                        mt8127_power_gs_3g_paging, mt8127_power_gs_3g_paging_len, \
...
#else
static int dump_3g_paging_read(struct file *flip, char __user *buf, size_t count, loff_t *offset)
{
    int len = 0;
    char *p = buf;
...
static const struct file_operations paging_3g_proc_fops = {
    .owner = THIS_MODULE,
    .read = dump_3g_paging_read,
};
#endif
...
static int __init mt_power_gs_3g_paging_init(void)
{
    ...
    #else
        mt_entry = proc_create("dump_3g_paging", S_IRUGO | S_IWUSR | S_IWGRP, mt_power_gs_dir, &paging_3g_proc_fops);
        if (!mt_entry) 
        {
            printk("[%s]: create proc file /proc/mt_power_gs/dump_3g_paging failed\n", __FUNCTION__);
        }
    #endif
    }
...
MODULE_DESCRIPTION("MT8127 Power Golden Setting - 3G Paging");

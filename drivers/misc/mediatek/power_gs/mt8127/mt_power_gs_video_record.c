...
extern unsigned int *mt8127_power_gs_video_record;
extern unsigned int mt8127_power_gs_video_record_len;
...
void mt_power_gs_dump_video_record(void)
{
    mt_power_gs_compare("Video Record",                                                 \
                        mt8127_power_gs_video_record, mt8127_power_gs_video_record_len, \
...
#else
static int dump_video_record_read(struct file *flip, char __user *buf, size_t count, loff_t *offset)
{
    int len = 0;
    char *p = buf;
...
static const struct file_operations video_record_proc_fops = {
    .owner = THIS_MODULE,
    .read = dump_video_record_read,
};
#endif
...
static int __init mt_power_gs_video_record_init(void)
{
    ...
    #else
        mt_entry = proc_create("dump_video_record", S_IRUGO | S_IWUSR | S_IWGRP, mt_power_gs_dir, &video_record_proc_fops);
        if (!mt_entry) 
        {
            printk("[%s]: create proc file /proc/mt_power_gs/dump_video_record failed\n", __FUNCTION__);
        }
    #endif
    }
...
MODULE_DESCRIPTION("MT8127 Power Golden Setting - Video Record");
